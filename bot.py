...
print(f"🔑 Giphy API Key: {config.GIPHY_API_KEY[:20]}... (длина: {len(config.GIPHY_API_KEY)})")
print(f"🔑 Unsplash API Key: {config.UNSPLASH_ACCESS_KEY[:20]}... (длина: {len(config.UNSPLASH_ACCESS_KEY)})")
print("=" * 60)
...
async def image_command(interaction: discord.Interaction, query: str):
    ...
    params = {
        ...
        "per_page": 10,
        "lang": "ru",
        "order_by": "relevant"  # Самые релевантные
    }
